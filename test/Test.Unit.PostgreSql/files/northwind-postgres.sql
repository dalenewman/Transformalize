...
-- if you just use xmin, we have to cast it to text and then bigint before comparing it
-- which is probably pretty slow in big tables
-- here is an alternate way to do it below but adds trigger overhead
-- best case is to probably make your app do the timestamps on insert and delete.

ALTER TABLE order_details ADD COLUMN modified_at TIMESTAMPTZ DEFAULT clock_timestamp();
CREATE INDEX idx_order_details_modified_at ON order_details (modified_at);
...
CREATE TRIGGER trg_order_details_modified_at
