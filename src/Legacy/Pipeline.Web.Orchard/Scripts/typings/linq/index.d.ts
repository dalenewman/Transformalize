declare namespace Enumerable {
  ...
  function from(): IEnumerable<any>; // empty
  function from<T>(obj: IEnumerable<T>): IEnumerable<T>;
  function from(obj: number): IEnumerable<number>;
  function from(obj: boolean): IEnumerable<boolean>;
  function from(obj: string): IEnumerable<string>;
  function from<T>(obj: T[]): IEnumerable<T>;
  function from<T>(obj: { length: number;[x: number]: T; }): IEnumerable<T>;
  function from(obj: any): IEnumerable<{ key: string; value: any }>;
  function make<T>(element: T): IEnumerable<T>;
  function matches<T>(input: string, pattern: RegExp): IEnumerable<T>;
  function matches<T>(input: string, pattern: string, flags?: string): IEnumerable<T>;
