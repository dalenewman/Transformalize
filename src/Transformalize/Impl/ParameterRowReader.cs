...
namespace Transformalize.Impl {
   ...
   public class ParameterRowReader : IReadStream, IRead {
      ...
      public async IAsyncEnumerable<IRow> ReadStreamAsync([EnumeratorCancellation] CancellationToken token = default) {
         ...
         IRow row;
         await using (var source = _parentReader.ReadStreamAsync(token).GetAsyncEnumerator(token)) {
            row = await source.MoveNextAsync().ConfigureAwait(false) ? source.Current : null;
         }
         if (row == null) {
            await using var fallback = _defaultRowReader.ReadStreamAsync(token).GetAsyncEnumerator(token);
            if (!await fallback.MoveNextAsync().ConfigureAwait(false)) yield break;
