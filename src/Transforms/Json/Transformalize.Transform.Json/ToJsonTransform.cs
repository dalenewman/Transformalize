...
namespace Transformalize.Transforms.Json {
   ...
   public class ToJsonTransform : BaseTransform {
      ...
      public ToJsonTransform(IContext context = null) : base(context, "string") {
         ...
         _formatting = Context.Operation.Format.ToLower() == "indented" ? Formatting.Indented : Formatting.None;
      }
      ...
      public override IRow Operate(IRow row) {
         ...
         foreach (var field in _input) {
            jsonObject[field.Alias] = JToken.FromObject(row[field]);
         }

         row[Context.Field] = JsonConvert.SerializeObject(jsonObject, _formatting);
         return row;
      }
