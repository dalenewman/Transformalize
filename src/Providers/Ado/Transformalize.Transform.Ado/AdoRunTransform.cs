...
namespace Transformalize.Transforms.Ado {
    public class AdoRunTransform : BaseTransform {
        ...
        public override async global::System.Collections.Generic.IAsyncEnumerable<IRow> OperateStreamAsync(
           ...
           if (GetType().GetMethod(nameof(Operate), new[] { typeof(IEnumerable<IRow>) }).DeclaringType != typeof(AdoRunTransform)) {
              await foreach (var row in base.OperateStreamAsync(rows, token).ConfigureAwait(false)) yield return row;
              yield break;
           }
           // The connection stays open for the whole enumeration and is disposed when it ends or the
           // consumer stops early.
           using (_cn) {
              if (_cn is global::System.Data.Common.DbConnection dbCn) {
                 await dbCn.OpenAsync(token).ConfigureAwait(false);
              } else {
                 _cn.Open();
              }
              await foreach (var row in rows.WithCancellation(token).ConfigureAwait(false)) {
                 ...
                 try {
                    var rowCount = await _cn.ExecuteAsync(new CommandDefinition(cmd, _parameters, cancellationToken: token)).ConfigureAwait(false);
                    Context.Debug(() => $"{cmd} affected {rowCount} row(s).");
                 } catch (Exception e) {
                    Context.Error(e, e.Message);
                    Run = false;
