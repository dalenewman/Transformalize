...
namespace Transformalize.Providers.Json {
   ...
   public class JsonFileReader : IReadStream, IRead {
      ...
      public IEnumerable<IRow> Read() {
         ...
         using (var stream = new FileStream(fileInfo.FullName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
            return new JsonStreamReader(_context, stream, _rowFactory).Read().ToList();
         }
      }
      ...
      public async IAsyncEnumerable<IRow> ReadStreamAsync([EnumeratorCancellation] CancellationToken token = default) {
         ...
         await foreach (var row in new JsonStreamReader(_context, stream, _rowFactory).ReadStreamAsync(token).WithCancellation(token).ConfigureAwait(false)) {
            yield return row;
         }
      }
      ...
      public async Task<IEnumerable<IRow>> ReadAsync(CancellationToken token = default) {
         ...
         var fileInfo = FileUtility.Find(_context.Connection.File);
         using (var stream = new FileStream(fileInfo.FullName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 4096, FileOptions.Asynchronous | FileOptions.SequentialScan)) {
            var rows = await new JsonStreamReader(_context, stream, _rowFactory).ReadAsync(token).ConfigureAwait(false);
            return rows.ToList();
         }
      }
   }
