...
namespace Transformalize.Libs.Lucene.Net.Search
{
	...
	public interface Searchable : IDisposable
	{
		...
		Document.Document Doc(int i);

        /// <summary> Get the <see cref="Document" />at the <c>n</c><sup>th</sup> position. The <see cref="FieldSelector"/>
		/// may be used to determine what <see cref="Field" />s to load and how they should be loaded.
		/// 
		/// <b>NOTE:</b> If the underlying Reader (more specifically, the underlying <c>FieldsReader</c>) is closed before the lazy <see cref="Field" /> is
		/// loaded an exception may be thrown.  If you want the value of a lazy <see cref="Field" /> to be available after closing you must
		...
		/// </param>
		/// <param name="fieldSelector">The <see cref="FieldSelector" /> to use to determine what Fields should be loaded on the Document.  May be null, in which case all Fields will be loaded.
		/// </param>
		/// <returns> The stored fields of the <see cref="Document" /> at the nth position
		/// </returns>
		/// <throws>  CorruptIndexException if the index is corrupt </throws>
		/// <throws>  IOException if there is a low-level IO error </throws>
		/// <summary> 
		...
		/// </seealso>
		Document.Document Doc(int n, FieldSelector fieldSelector);
