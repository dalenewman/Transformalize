...
{
    /// <summary>
    ///     This class only have <b>static methods</b> to work with files and strings (the most common of them)
    /// </summary>
    ...
    /// <param name="records">The records to write (Can be an array, List&lt;T&gt;, etc)</param>
        ...
        {
            var engineRead = new FileHelperAsyncEngine(recordType);
            var engineWrite = new FileHelperAsyncEngine(recordType);
        ...
        public static object[] MergeAndSortFile(Type recordType, string file1, string file2, string destFile, string field, bool ascending)
        {
            ...
            var res = (object[]) arr.ToArray(recordType);
