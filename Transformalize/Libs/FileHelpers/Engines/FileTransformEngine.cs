...
#endif
        private static readonly object[] mEmptyArray = new object[] {};

#if NET_2_0
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
#endif
        private readonly Type mSourceType;
#if NET_2_0
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
#endif
        private readonly Type mDestinationType;
...
//		}
...
#endif
        {
            ...
            var engine = new FileHelperAsyncEngine(mSourceType, mSourceEncoding);
		...
		private Destination[] CoreTransformFile(string sourceFile, string destFile, Type sourceType, Type destType, MethodInfo method)
		{
            ...
            using (var fs = new StreamReader(sourceFile, mSourceEncoding, true))
            {
                using (var ds = new StreamWriter(destFile, false, mDestinationEncoding))
                {
        ...
        {
            var sourceEngine = new FileHelperAsyncEngine(sourceType);
            var destEngine = new FileHelperAsyncEngine(destType);
        ...
        private MethodInfo GetTransformMethod(Type sourceType, Type destType)
        {
            ...
            foreach (var m in methods)
            {
                ...
                {
                    var ta = (TransformToRecordAttribute) m.GetCustomAttributes(typeof (TransformToRecordAttribute), false)[0];
