...
namespace Transformalize.Libs.Ninject.Infrastructure.Language
{
    internal static class ExtensionsForIEnumerable
    {
        ...
        {
            MethodInfo method = typeof (Enumerable).GetMethod("Cast").MakeGenericMethod(elementType);
        ...
        {
            MethodInfo method = typeof (Enumerable).GetMethod("ToArray").MakeGenericMethod(elementType);
            return method.Invoke(null, new[] {series}) as Array;
        }

        public static IList ToListSlow(this IEnumerable series, Type elementType)
        {
            MethodInfo method = typeof (Enumerable).GetMethod("ToList").MakeGenericMethod(elementType);
