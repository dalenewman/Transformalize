    ...
    public class EntityTransformParametersReader : ITransformParametersReader
    {
        ...
        public Parameters Read(TransformConfigurationElement transform)
        {
            ...
            foreach (ParameterConfigurationElement p in transform.Parameters)
            {
                ...
                {
                    _log.Warn(
                        "The entity {0} has a {1} transform parameter without a field attribute, or name and value attributes.  Entity parameters require one or the other.",
                        _entity.Alias, transform.Method);
                ...
                if (!string.IsNullOrEmpty(p.Field))
                {
                    ...
                    {
                        _log.Warn(
                            "The entity {0} has a {1} transform parameter that references field {2}.  This field doesn't exist in {0}.",
                            _entity.Alias, transform.Method, p.Field);
                        return new Parameters();
                    }
                }
                else
