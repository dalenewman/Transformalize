...
namespace Transformalize.Main.Providers.ElasticSearch
{
    public class ElasticSearchEntityOutputKeysExtract : AbstractOperation {
        ...
        public ElasticSearchEntityOutputKeysExtract(AbstractConnection connection, Entity entity) {
            ...
            _entity = entity;
            _aliasTypes = _entity.PrimaryKey.Select(f => new AliasType() { Alias = f.Value.Alias.ToLower(), SimpleType = f.Value.SimpleType }).ToList();
            if (_entity.Version != null) {
                _aliasTypes.Add(new AliasType() { Alias = _entity.Version.Alias.ToLower(), SimpleType = _entity.Version.SimpleType });
            }
