...
namespace Transformalize.Transforms {
    public class TrimEndTransform : StringTransform {
        private readonly Field _input;
        private readonly char[] _trimChars;

        public TrimEndTransform(IContext context = null) : base(context, "string") {
        ...
        public override IRow Operate(IRow row) {
            row[Context.Field] = GetString(row, _input).TrimEnd(_trimChars);

            return row;
        }

        public override IEnumerable<OperationSignature> GetSignatures() {
            yield return new OperationSignature("trimend") {
