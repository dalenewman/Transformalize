   ...
   public class BetterFormat {
      ...
      public BetterFormat(IContext context, string format, Func<IEnumerable<Field>> fallback) {
         ...
         foreach (Match match in matches) {
            if(match.Index >= 2) {
               if(format.Substring(match.Index-2,2) == "{{") {
                  continue;  // we do not want to try and replace when brackets are doubled
               }
            }
         ...
         if (numeric) {
            fields.AddRange(fallback().Where(f=>!f.System)); // receiving un-named fields
         } else {
            var count = 0;
