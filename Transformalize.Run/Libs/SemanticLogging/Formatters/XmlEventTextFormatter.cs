...
namespace Transformalize.Run.Libs.SemanticLogging.Formatters
{
    /// <summary>
        ...
        private static void XmlWritePayload(XmlWriter writer, EventEntry entry)
        {
            ...
            for (int i = 0; i < entry.Payload.Count; i++)
            {
                ...
                catch (Exception e)
                {
                    ...
                    throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Resources.XmlSerializationError, e.Message), e);
                }
            }
        }
