...
using Transformalize.Run.Libs.SemanticLogging.Schema;
using Transformalize.Run.Libs.SemanticLogging.Utility;

namespace Transformalize.Run.Libs.SemanticLogging.EventSourceAnalyzer
{
    /// <summary>
    ...
    public sealed class EventSourceAnalyzer
    {
        ...
        private ICollection<EventSchema> GetEventSchemas(EventSource eventSource)
        {
            ...
            {
                throw new EventSourceAnalyzerException(string.Format(CultureInfo.CurrentCulture, Resources.EventSourceAnalyzerManifestGenerationError, e.Message, EventSource.GenerateManifest(eventSource.GetType(), null)));
            }
        }
        ...
        private void CheckForBadFormedManifest(string manifest)
        {
            ...
            {
                throw new EventSourceAnalyzerException(Resources.EventSourceAnalyzerBadFormedManifestError);
            }
        }
        ...
        private class ProbeEventListener : EventListener
        {
            ...
            private void CheckParametersOrder()
            {
                ...
                if (this.EventData.Payload[this.TypeOrderOffset - payloadParameterOffset].IsDefault())
                {
                    this.Error = new EventSourceAnalyzerException(string.Format(CultureInfo.CurrentCulture,
                        Resources.EventSourceAnalyzerMismatchParametersOrder,
            ...
            private void CheckPayload()
            {
                ...
                {
                    this.Error = new EventSourceAnalyzerException(string.Format(CultureInfo.CurrentCulture, Resources.EventSourceAnalyzerDifferentParameterCount, eventMethod.Name));
                    return;
                }
                ...
                for (int i = 0; i < this.EventData.Payload.Count; i++)
                {
                    if (this.EventData.Payload[i] == null)
                    {
                        ...
                        this.Error = new EventSourceAnalyzerException(string.Format(CultureInfo.CurrentCulture,
                            Resources.EventSourceAnalyzerNullPayloadValue,
                            i,
                    ...
                    {
                        this.Error = new EventSourceAnalyzerException(string.Format(CultureInfo.CurrentCulture,
                            Resources.EventSourceAnalyzerMismatchParametersType,
                            eventParameters[i + payloadParameterOffset].Name,
